...
// from "Practical Hash-based Owen Scrambling"
// https://jcgt.org/published/0009/04/01/
inline uint Sobol(uint index, uint dim)
{
	static const uint directions[5][32] = {
...
inline uint hash_combine(uint seed, uint v)
{
...
inline uint laine_karras_permutation(uint x, uint seed) {
	x += seed;
	x ^= x * 0x6c50b47cu;
	x ^= x * 0xb82f1e52u;
	x ^= x * 0xc7afe638u;
...
inline uint nested_uniform_scramble_base2(uint x, uint seed) {
	x = reverse_bits(x);
...
inline uint4 ShuffledScrambledSobol(uint index, uint seed)
{
	index = nested_uniform_scramble_base2(index, seed);
	uint4 ret = { 0, 0, 0, 0 };
	for (uint i = 0; i < 4; ++i)
		ret[i] = nested_uniform_scramble_base2(Sobol(index, i), hash_combine(seed, i));
	return ret;
}
